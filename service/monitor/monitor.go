...
func sendRequestToAliMonitor(metrics []AliyunMetric) error {
	...
	if err != nil {
		log.Println("http.NewRequest failed", map[string]interface{}{"err": err.Error(), "url": url, "body": body})
		return err
	}
	...
	if err != nil || nil == resp {
		log.Println("http client.Do failed", map[string]interface{}{"err": errDo, "url": url, "body": body})
		return errDo
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		log.Println("http resp status error", map[string]interface{}{"status": resp.StatusCode, "url": url, "body": body})
		return errors.New("http response status error")
	}
	_, errRead := ioutil.ReadAll(resp.Body)
	if errRead != nil {
		log.Println("read http response failed", map[string]interface{}{"err": errRead, "url": url, "body": body})
	}
	return nil
}
...
func sendMetricsToStatsd(metrics []string) error {
	...
	if err != nil {
		log.Println("conn statsd failed", map[string]interface{}{"StatsdAddr": monitorObj.conf.StatsdAddr, "error": err})
		return err
	}
	...
	for index := range metrics {
		...
		if err != nil {
			log.Println("Send statsd metric failed", map[string]interface{}{"content": metrics[index], "error": err})
		}
	}
	return nil
}
