...
func unexpectError(format string, args ...interface{}) {
	panic(fmt.Errorf("[urpcsrv] "+format, args...))
}

// ErrorRsp 生成GRPC错误返回
func newErrorRsp(mcode string, format string, args ...interface{}) *urpccomm.CommResponse {
...
func newRspFromError(err error) *urpccomm.CommResponse {
...
func createRegInfo(reqBody interface{}, f interface{}) *RegisterInfo {
	...
	for i := 0; i < ft.NumIn(); i++ {
		...
		name := inType.String()
		// 请求透传
		if name == "urpccomm.CommRequest" {
			unexpectError("urpccomm.CommRequest must be a pointer")
		}
	...
	if numOut == 2 {
		...
		for {
			...
			if ft.Out(0).String() == "*urpccomm.CommResponse" {
...
func call0(ctx context.Context, headerValue, bodyValue reflect.Value, commReq *urpccomm.CommRequest, regInfo *RegisterInfo) *urpccomm.CommResponse {
	// 调用函数
	var (
	...
	if rspCommRsp.IsValid() && !rspCommRsp.IsNil() {
		return rspCommRsp.Interface().(*urpccomm.CommResponse)
	}
...
func newCommRequest() *urpccomm.CommRequest {
	// FIXME: 使用内存池来减少堆分配
	return &urpccomm.CommRequest{}
}

func putCommRequest(v *urpccomm.CommRequest) {
	// TODO:
}

func (d *decoder) Decode() (*urpccomm.CommRequest, error) {
	commReq := newCommRequest()
	err := proto.Unmarshal(d.buf, commReq)
	if err != nil {
		putCommRequest(commReq)
		return nil, err
	}
